#This script pieces together a file of names for the gender_guesser_mod, based on the original
#dictionary provided by the author, along with additional name-gender pairs as needed.
#Some names have an unknown gender, as I could not find them on ssb.no/names.

#Loading the source-files and retrieving the data
header_file = open("gender_guesser_mod/data/header.txt", "r", encoding="utf-8")
header_string = header_file.read()

base_file = open("gender_guesser_mod/data/base.txt", "r", encoding="utf-8")
base_list = base_file.readlines()

new_lines_file = open("new_names.txt", "r", encoding="utf-8")

new_dict_file = open("gender_guesser_mod/data/nam_dict.txt", "w+", encoding="utf-8")
